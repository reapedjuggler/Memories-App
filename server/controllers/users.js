	...
	try {
		...
		if (isEmailRegisteredResp.length === 0) {
			...
			const isPasswordCorrect = bcrypt.compare(
				password,
				isEmailRegisteredResp.password
			);
			...
			if (isPasswordCorrect === false) {
				...
				const token = jwt.sign(
					{ email: isEmailRegisteredResp.email, id: isEmailRegisteredResp._id },
					privateKey,
					{ expiresIn: "1h" }
				);
				...
				return res
					.status(200)
					.json({ resp: isEmailRegisteredResp, token: token });
			}
		}
	} catch (err) {
	...
	try {
		...
		if (isAlreadyReg.length) {
			...
			return res
				.status(409)
				.json(
					"Another user exist with the given email id, Kindly Log In if you're already registered"
				);
		}
        ...
        */
		...
		const token = jwt.sign({ email: resp.email, id: resp._id }, privateKey, {
			expiresIn: "1h",
		});
		...
		res
			.status(500)
			.json({ message: "F! Something Went Wrong, I'ma sleep Again" });
	}
};
